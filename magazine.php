...
$clothes = [];
foreach($goods as $good){
  array_push($clothes, $good['category']);
}
$clothe = array_unique($clothes);
...
if (isset($_SESSION['user'])){
  ...
  echo "<a href='favourites.php'>Ваши избранные товары</a><br>";
  $auth = True;
...
<?php
  ...
  if(isset($_GET['delindex'])){
    ...
    if (!empty($_POST['category'])){
      array_push($goods, ['name'=>$_POST['name'], 'description'=> $_POST['description'], 'category'=>$_POST['category'], 'price'=>$_POST['price'], 'imageUrl'=>'img/'.$_POST['img'], 'stock'=>$_POST['stock'], 'offer'=>$_POST['offer']]);
    } else {
      array_push($goods, ['name'=>$_POST['name'], 'description'=> $_POST['description'], 'category'=>$_POST['select_category'], 'price'=>$_POST['price'], 'imageUrl'=>'img/'.$_POST['img'], 'stock'=>$_POST['stock'], 'offer'=>$_POST['offer']]);
    }
    usort($goods, function($a, $b) {return strcmp($b["category"], $a["category"]);});
    file_put_contents('goods.json', json_encode($goods));
